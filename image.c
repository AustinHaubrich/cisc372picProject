...
void* threadedConvolute(void* obj){
    convolute_t* imgData=(convolute_t*)obj;
    long my_rank = (long) imgData->rank;
    int row,pix,bit,span;
    ...
    for (row=my_first_row;row<my_last_row;row++){
        for (pix=0;pix<imgData->srcImage->width;pix++){
            for (bit=0;bit<imgData->srcImage->bpp;bit++){
                imgData->destImage->data[Index(pix,row,imgData->srcImage->width,bit,imgData->srcImage->bpp)]=getPixelValue(imgData->srcImage,pix,row,bit,imgData->algorithm);
            }
        }
    }
}
